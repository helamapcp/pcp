...
interface TransferModalProps {
  ...
  onClose: () => void;
  onSubmit: (quantity: number, from: 'CD' | 'Factory' | 'PMP', to: 'CD' | 'Factory' | 'PMP') => void;
}

type Sector = 'CD' | 'Factory' | 'PMP';
...
export function TransferModal({
  ...
  const [to, setTo] = useState<Sector>('Factory');
  const [error, setError] = useState('');

  if (!isOpen || !product) return null;

  const sectors: Sector[] = ['CD', 'Factory', 'PMP'];
