  ...
  if (!loaded) {
    return (
      <div className="px-4 py-2">
        <button onClick={load} className="text-primary text-xs font-bold hover:underline">Carregar itens...</button>
      </div>
    );
  }
...
function getTabFromPath(path: string): 'users' | 'products' | 'formulations' {
  if (path === '/admin/products') return 'products';
  if (path === '/admin/formulations') return 'formulations';
  return 'users';
}
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    fetchUsers(); fetchCategories(); fetchProducts(); fetchFormulations();
  }, [fetchUsers, fetchCategories, fetchProducts, fetchFormulations]);
  ...
  const handleAddUser = async () => {
    if (!userFormData.name || !userFormData.email || !userFormData.password) { toast.error('Preencha Nome, E-mail e Senha'); return; }
  ...
  const handleSaveProduct = async () => {
    ...
    const payload = {
      name: productFormData.name, category_id: productFormData.category_id,
      unit_weight_kg: parseFloat(productFormData.unit_weight_kg) || 0, base_unit: productFormData.base_unit,
      conversion_factor: parseFloat(productFormData.conversion_factor) || 1,
      package_type: productFormData.package_type, package_weight: parseFloat(productFormData.package_weight) || 0,
    };
    if (editingProductId) { await supabase.from('products').update(payload).eq('id', editingProductId); toast.success('Produto atualizado!'); }
    else { await supabase.from('products').insert(payload); toast.success('Produto criado!'); }
  ...
  const handleEditProduct = (p: ProductItem) => {
    setProductFormData({ name: p.name, category_id: p.category_id || '', unit_weight_kg: String(p.unit_weight_kg), base_unit: p.base_unit, conversion_factor: String(p.conversion_factor), package_type: p.package_type, package_weight: String(p.package_weight) });
    setEditingProductId(p.id); setShowProductForm(true);
  };
  const handleDeleteProduct = async (id: string) => { await supabase.from('products').delete().eq('id', id); toast.success('Produto removido'); fetchProducts(); };
  const handleSaveCategory = async () => {
    if (!categoryFormData.trim()) return;
    if (editingCategoryId) { await supabase.from('categories').update({ name: categoryFormData }).eq('id', editingCategoryId); }
    else { await supabase.from('categories').insert({ name: categoryFormData }); }
    toast.success(editingCategoryId ? 'Atualizada!' : 'Criada!');
    setCategoryFormData(''); setShowCategoryForm(false); setEditingCategoryId(null); fetchCategories(); fetchProducts();
  };
  const handleDeleteCategory = async (id: string) => { await supabase.from('categories').delete().eq('id', id); toast.success('Removida'); fetchCategories(); };
  const handleSaveFormulation = async () => {
    if (!formulationFormData.name || !formulationFormData.final_product) { toast.error('Preencha nome e produto final'); return; }
    const payload = { name: formulationFormData.name, final_product: formulationFormData.final_product, machine: formulationFormData.machine || null, weight_per_batch: parseFloat(formulationFormData.weight_per_batch) || 0 };
    if (editingFormulationId) { await supabase.from('formulations').update(payload).eq('id', editingFormulationId); toast.success('Formulação atualizada!'); }
    else { await supabase.from('formulations').insert(payload); toast.success('Formulação criada!'); }
  ...
  const handleEditFormulation = (f: FormulationItem) => {
    setFormulationFormData({ name: f.name, final_product: f.final_product, machine: f.machine || '', weight_per_batch: String(f.weight_per_batch) });
  ...
  const handleAddFormulationItem = async () => {
    ...
    await supabase.from('formulation_items').insert({ formulation_id: showDetailForm, product_id: detailFormData.product_id, quantity_per_batch: parseFloat(detailFormData.quantity_per_batch) || 0 });
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto w-full">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-black text-foreground">
          ...
          {activeTab === 'formulations' && '🧪 Formulações'}
        </h1>
        <p className="text-muted-foreground text-sm">
          {activeTab === 'users' && 'Gerenciamento de usuários do sistema'}
          {activeTab === 'products' && 'Cadastro de produtos e categorias'}
          {activeTab === 'formulations' && 'Receitas de produção'}
        </p>
      </div>
          ...
          {showFormulationForm && (
            <AdminModal title={editingFormulationId ? '✏️ Editar Formulação' : '➕ Nova Formulação'} onClose={() => { setShowFormulationForm(false); setEditingFormulationId(null); }}>
              ...
              <div>
                ...
                <input list="final-products-list" value={formulationFormData.final_product}
                ...
                <datalist id="final-products-list">
                  {[...new Set(formulations.map(f => f.final_product))].map(fp => <option key={fp} value={fp} />)}
                </datalist>
              </div>
              <div>
                <label className="text-foreground font-bold text-xs mb-1 block">Máquina (opcional)</label>
                <select value={formulationFormData.machine}
                  ...
                  {mixers.filter(m => m.active).map(m => <option key={m.id} value={m.name}>{m.name} ({m.capacity_kg}kg)</option>)}
