...
-- Update confirm_production to handle excess consumption and creation
  ...
  LOOP
    ...
    IF abs(v_adj_kg - v_ideal_kg) > 0.001 THEN
      IF v_pkg_type = 'sealed_bag' AND v_pkg_weight > 0 THEN
        -- Natural rounding check: ideal - excess_used = net, ceil(net/pkg)*pkg = adj
        DECLARE
    ...
    IF v_excess_used > 0 THEN
      ...
      LOOP
        ...
        ELSE
          -- Partially consume: reduce excess_kg
    ...
    -- Validate stock sufficiency (only deducting sack amount from PCP)
