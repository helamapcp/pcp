...
-- 1. Add justification column to production_order_items for manual overrides
ALTER TABLE production_order_items
ADD COLUMN IF NOT EXISTS justification text DEFAULT NULL;

-- 2. Replace confirm_production RPC with hardened version
...
BEGIN
  ...
  -- 4. Process each item with row locks and server-side validation
  ...
  LOOP
    ...
    -- SERVER-SIDE VALIDATION: justification required if override
    IF abs(v_adj_kg - v_ideal_kg) > 0.001 THEN
      -- Check if it's a natural rounding (sealed_bag ceil) vs manual override
      IF v_pkg_type = 'sealed_bag' AND v_pkg_weight > 0 THEN
        -- Natural rounding = ceil(ideal / pkg) * pkg
        IF abs(v_adj_kg - (ceil(v_ideal_kg / v_pkg_weight) * v_pkg_weight)) > 0.001 THEN
          -- Manual override beyond natural rounding
        ...
        END IF;
      ELSE
        -- Non-sealed_bag with difference requires justification
        IF v_justification IS NULL OR length(trim(v_justification)) = 0 THEN
          RAISE EXCEPTION 'Justificativa obrigatória para ajuste manual do produto %', v_item->>'product_id';
        END IF;
      END IF;
    ...
    -- Insert order item with justification
    ...
    -- Lock and read current PCP stock (FOR UPDATE prevents race conditions)
    ...
    INSERT INTO stock_movements (
      ...
      format('Produção %s (%s) - %s batidas | Lote: %s | Ideal: %skg | Enviado: %skg%s',
